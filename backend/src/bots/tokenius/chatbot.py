...
from bots.tokenius.localization import get_localization
from bots.tokenius.localization.base_local import BaseLocalization
...
VECTOR_STORE_DIRECTORY = "btsllm"
STRUCTURE_ID = "btsllm"
embeddings = LiteLLMEmbedding()
chain = RetrievalChain()


class Tokenius(BaseBot):
    """Bot for assisting in learning of Secrets behind LLMs."""

    def __init__(self):
        """Initialize Tokenius."""
        super().__init__(
            Topic.tokenius.value,
            LocalizedString("Tokenius", "Tokenius"),
            LocalizedString(
                "Your Buddy for the Lecture Behind the Secrets of Large Language " \
                "Models",
                (
                    "Dein Begleiter zur Vorlesung Behind the Secrets of Large Language "
                    "Models"
                ),
            ),
            LocalizedString(
                "Your Buddy for the Lecture Behind the Secrets of Large Language Models"
                " - providing you with all kinds of information "
                "about the lecture, exercises, and organizational matters.",
                "Dein Begleiter zur Vorlesung Behind the Secrets of Large " \
                "Language Models"
            ...
            color="#2969e8",
            avatar="/static/images/{theme}/tokenius.png",
            tag="INF",
        )
    ...
    async def init_chat(self, request: RequestModel) -> Response:
        ...
        logger.info("Tokenius: Initializing chat")
    ...
    async def chat_invoke(self, request: RequestModel) -> Response | StreamingResponse:
        ...
        logger.info("Tokenius: Processing chat request")
    ...
    async def update_title(self, request: RequestModel) -> TitleResponse:
        ...
        logger.info("Tokenius: Generating title")
    ...
    async def get_response(
        ...
        logger.info("Tokenius: Preparing response")
...
def load_module_db(module: str, locale: LocaleType) -> VectorStore:
    ...
    if not module:
        logger.info("Tokenius: Loading vector storage (all)")
        return FAISSVectorDB().load_local(
            str(Path(VECTOR_STORE_DIRECTORY).parent / "all"),
            embeddings,
        )

    logger.info("Tokenius: Loading vector storage")
...
async def update_chain(
    ...
    logger.info("Tokenius: Updating chain")
...
# TODO: create prompts and rename them
    if "tokenius_role" in prompt.input_variables:
        role = requested_llm.get_langfuse_prompt(
            "tokenius-description-" + locale.value.lower(),
            label=label,
            prompt_type="text",
        ).format()
        prompt = prompt.partial(tokenius_role=role)
...
async def open_conversation_base(
    ...
    logger.info("Tokenius: Setting up conversation")

    vector_db = await tokenius.get_vector_store()  # TODO

    prompts = get_localized_langfuse_prompt(
        "tokenius-open-conversation",
        locale,
...
async def multi_language_retriever(
    ...
    """
    prompt = get_localized_langfuse_prompt(
        "tokenius-module-retriever-query",
        locale,
        chat=messages,
    )
    ...
    created_chain = (
        ...
        metadata={
            ...
            "langfuse_tags": ["tokenius", "multi-retriever"],  # TODO
        },
    )
    ...
    if isinstance(module_queries, dict) and "module" in module_queries:
        ...
        logger.info(f"Tokenius: Detected module from queries: {detected_module}")
    ...
    if not docs:
        ...
        logger.info("Tokenius: No documents found. Trying again without threshold.")
        for query in queries:

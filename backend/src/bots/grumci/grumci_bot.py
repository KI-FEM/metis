...
from bots.grumci.localization import get_localization
...
VECTOR_STORE_DIRECTORY = "grumci"
STRUCTURE_ID = "grumci"
embeddings = LiteLLMEmbedding()
chain = RetrievalChain()


class GrumciBuddy(BaseBot):
    """Bot for assisting in learning of Human-Computer-Interactions."""

    def __init__(self):
        """Initialize the GrumciBuddy."""
        super().__init__(
            Topic.grumci_buddy.value,
            LocalizedString("Gery", "Gery"),
            LocalizedString(
                "Your Buddy for the Introduction to Human-Computer-Interaction Lecture",
                (
                    "Dein Begleiter zur Vorlesung Grundlagen der Mensch-Computer-"
                    "Interaktion"
                ),
            ),
            LocalizedString(
    ...
    async def init_chat(self, request: RequestModel) -> Response:
        ...
        logger.info("GrumciBuddy: Initializing chat")
    ...
    async def chat_invoke(self, request: RequestModel) -> Response | StreamingResponse:
        ...
        logger.info("GrumciBuddy: Processing chat request")
    ...
    async def update_title(self, request: RequestModel) -> TitleResponse:
        ...
        logger.info("GrumciBuddy: Generating title")
        ...
        title_response = (
            ...
            .create_chain(
                llm_client.llm(max_tokens=200, chat_id=request.find_store("chat_id")),
                prompt,
            )
            .ainvoke({})
        )
    ...
    async def get_response(
        ...
        logger.info("GrumciBuddy: Preparing response")
...
grumci_buddy = GrumciBuddy()
post = grumci_buddy.post
...
async def update_chain(
    ...
    logger.info("GrumciBuddy: Updating chain")
...
def get_localized_langfuse_prompt(
    ...
    if "grumci_role" in prompt.input_variables:
        role = requested_llm.get_langfuse_prompt(
            "grumci-description-" + locale.value.lower(),
            label=label,
            prompt_type="text",
        ).format()
        prompt = prompt.partial(grumci_role=role)
...
async def open_conversation_base(
    ...
    logger.info("GrumciBuddy: Setting up conversation")

    vector_db = await grumci_buddy.get_vector_store()  # TODO

    prompts = get_localized_langfuse_prompt(
        "grumci-open-conversation",
        locale,
...
async def multi_language_retriever(
    ...
    created_chain = (
        ...
        metadata={
            ...
            "langfuse_tags": ["grumci-buddy", "multi-retriever"],  # TODO
        },
    )
    ...
    if isinstance(module_queries, dict) and "module" in module_queries:
        ...
        logger.info(f"GrumciBuddy: Detected module from queries: {detected_module}")

        competence = await db.get_competence_by_code(f"grumci_{detected_module}")
    ...
    if not docs:
        ...
        logger.info("GrumciBuddy: No documents found. Trying again without threshold.")
        for query in queries:

...
class TestDissy(BaseBotTest):
    """Test class for Dissy grouping tests together."""

    def setup_method(self):
        """Setup the bot for testing."""
        self.base_route = "dissy"
    ...
    def test_default_endpoint(self, client, mocker):
        ...
        assert response.json() == {
            ...
            "messages": [
                {
                    "buttons": [],
                    "content": "Hallo 👋 Ich bin _Dissy_ und ich bin hier, um dir "
                    "dabei zu helfen, die Konzepte über verteilte Systeme zu "
                    "verstehen. Ich freue mich darauf, dein Assistent und Begleiter "
    ...
    def test_default_endpoint_english(self, client, mocker):
        ...
        assert response.json() == {
            ...
            "messages": [
                {
                    "buttons": [],
                    "content":  "Hello 👋 I am _Dissy_ and I'm here to help you learn "
                    "and understand concepts about distributed Systems. I'm excited to "
                    "be your assistant and guide throughout this journey. "
    ...
    def test_open_conversation(self, client, fake_llm, mocker):
        ...
        mocker.patch("bots.dissy.chatbot.multi_language_retriever", return_value=[])
        mocker.patch(
            "bots.dissy.chatbot.load_module_db",
        ...
        PromptPatcher().add_prompt(
            ...
            name="dissy-description-en",
            prompt_type="text",
        ).add_prompt(
            prompt=[
                {"content": "retriever prompt", "role": "system"},
            ],
            name="dissy-module-retriever-query-en",
            prompt_type="chat",
        ).patch(mocker)
        response = self.post_request(
    ...
    def test_get_localized_langfuse_prompt(self, mocker, monkeypatch):
        ...
        from bots.dissy.chatbot import get_localized_langfuse_prompt
    ...
    async def test_update_chain(self, mocker):
        ...
        from bots.dissy.chatbot import update_chain
        self.mock_db(mocker)

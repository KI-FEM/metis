...
class TestTokenius(BaseBotTest):
    """Test class for Tokenius grouping tests together."""

    def setup_method(self):
        """Setup the bot for testing."""
        self.base_route = "tokenius"

    def test_default_endpoint(self, client, mocker):
        ...
        assert response.json() == {
            ...
            "messages": [
                {
                    "buttons": [],
                    "content": "Hallo 👋 Ich bin _Tokenius_ und ich bin hier, "
                    "um dir dabei zu helfen, die Konzepte und Geheimnisse von "
                    "Large Language Models zu lernen und zu verstehen. "
                    "Ich freue mich darauf, dein Assistent und Begleiter auf "
    ...
    def test_default_endpoint_english(self, client, mocker):
        """Test '/' endpoint when starting a fresh chat.
        ...
        assert response.json() == {
            ...
            "messages": [
                {
                    "buttons": [],
                    "content":  "Hello 👋 I am _Tokenius_ and I'm here to help you learn "
                    "and understand concepts and secrets of Large Language Models. "
    ...
    def test_open_conversation(self, client, fake_llm, mocker):
        ...
        mocker.patch("bots.tokenius.chatbot.multi_language_retriever", return_value=[])
        mocker.patch(
            "bots.tokenius.chatbot.load_module_db",
        ...
        PromptPatcher().add_prompt(
            ...
            name="tokenius-description-en",
            prompt_type="text",
        ).add_prompt(
            prompt=[
                {"content": "retriever prompt", "role": "system"},
            ],
            name="tokenius-module-retriever-query-en",
            prompt_type="chat",
        ).patch(mocker)
        response = self.post_request(
    ...
    def test_get_localized_langfuse_prompt(self, mocker, monkeypatch):
        ...
        from bots.tokenius.chatbot import get_localized_langfuse_prompt
    ...
    async def test_update_chain(self, mocker):
        ...
        from bots.tokenius.chatbot import update_chain
        self.mock_db(mocker)

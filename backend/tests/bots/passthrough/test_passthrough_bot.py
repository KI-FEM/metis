...
class TestPassthroughBot(BaseBotTest):
    ...
    def test_default_endpoint(self, client):
        ...
        assert response.json() == {
            "messages": [
                {
                    "content": "Welcome! You can freely chat with me here.",
                    "type": "assistant",
                    "buttons": [],
                    "meta_information": {
                        "sources": None,
    ...
    def test_open_conversation(self, client, mocker, fake_llm):
        ...
        PromptPatcher().add_prompt(
            prompt=[{"content": "Passthrough bot!", "role": "system"}],
            name="passthrough",
        ).patch(mocker)
    ...
    def test_update_title(self, client, mocker, fake_llm):
        ...
        self.mock_db(mocker)
        PromptPatcher().add_prompt(
            prompt=[{"content": "Generate title!", "role": "system"}], name="title-en"
        ).patch(mocker)
    ...
    def test_summarize_chat(self, client, mocker, fake_llm):
        ...
        PromptPatcher().add_prompt(
            ...
            prompt=[{"content": "Summarize pls!", "role": "system"}],
        ).patch(mocker)
        ...
        assert (
            response_data["keep_last_messages_until"] == 16
        )  # KEEP_LAST_MESSAGES_UNTIL
    ...
    def test_chat_uses_summary(self, client, mocker, fake_llm):
        ...
        test_data = {
            **open_conversation_data,
            "chat_history": [
                {
                    "content": "Message that should be summarized",
                    "type": "user",

<script setup lang="ts">
  ...
  function submitAction(button: ActionButton) {
...
<template>
  <div v-for="(message, index) in chatStore.currentChat?.messages"
       :key="index"
       :class="{'text-start': message.sender === 'assistant', 'text-end': message.sender === 'user'}">
    ...
    <ChatBubble :disabled="index !== (chatStore.currentChat?.messages.length ?? 1) - 1 "
                :message="message"
                @action="submitAction"

<script setup lang="ts">
  ...
  const selectedCitation = computed(() => {
    if (selectedSource.value == null) return null
    const i = selectedSource.value - 1
    return i >= 0 && i < citations.length ? citations[i] : null
  })
...
<template>
  ...
  <SourceInfoDialog v-if="selectedCitation"
